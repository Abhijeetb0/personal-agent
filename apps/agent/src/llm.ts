    ...
    try {
      if (p === "groq") {
        if (!process.env.GROQ_API_KEY) {
          console.log("[llm] groq key nahi, skip");
          continue;
        }
      ...
      if (p === "gemini") {
        if (!process.env.GEMINI_API_KEY) {
          console.log("[llm] gemini key nahi, skip");
          continue;
        }
        return await geminiChat(messages, maxTokens);
      }
      console.log(`[llm] unknown provider "${p}" skip`);
    } catch (e) {
      lastErr = e;
      console.error(`[llm] ${p} fail, agla provider:`, (e as Error).message.slice(0, 120));
    }
  }
